     ...
     <div class="icons">
        ...
        <div class="notif-wrap">
            ...
            <button type="button" class="icon notif-btn" id="notif-btn"
                ...
                <span class="notif-dot" id="notif-dot" aria-hidden="true" hidden></span>
            </button>
            ...
            <div class="notif-popover" id="notif-popover" role="menu" aria-labelledby="notif-btn">
                <div class="notif-header">
                    ...
                    <span class="notif-header-badge" id="notif-badge">
                ...
                <div class="notif-footer">
                    <a href="{{ route('profil') }}">Zobacz wszystkie powiadomienia</a>
                </div>
            </div>
        </div>
...
(function() { 
    ...
    function setBadge(unreadCount) {
        if (!badge) return;
        badge.textContent = unreadCount > 0 ? String(unreadCount) : '0';
        if (dot) dot.hidden = unreadCount <= 0;
    }

    function renderItem(n) {
        const isUrgent = n.severity === 'warning';
        const titleStyle = n.state ? '' : ' style="font-weight:700;"';

        return `
        <div class="notif-item${isUrgent ? ' urgent' : ''}" data-alert-id="${escapeHtml(n.id)}">
            <div class="notif-item-icon"></div>
            <div class="notif-item-body">
                <div${titleStyle}>${escapeHtml(notifLabel(n.type))}</div>
                <div style="font-size:12px;color:#4b5563;margin-top:2px;">${escapeHtml(n.description)}</div>
                <div style="font-size:10.5px;color:#9aa5ad;margin-top:2px;">${formatDate(n.createdAt)}</div>
            </div>
        </div>`;
    }
    ...
    async function loadNotifications() {
        ...
        try {
            ...
            setBadge(json.meta ? json.meta.unreadCount ?? 0 : 0);

            list.innerHTML = alerts.length
                ? alerts.slice(0, 8).map(renderItem).join('')
                : '<div style="padding:16px;color:#6b7280;font-size:13px;text-align:center;">Brak powiadomień.</div>';

            list.querySelectorAll('.notif-item[data-alert-id]').forEach((item) => {
                item.addEventListener('click', () => markRead(item.dataset.alertId));
            });
        } catch (error) {
            list.innerHTML = '<div style="padding:16px;color:#6b7280;font-size:13px;text-align:center;">Nie udało się pobrać powiadomień.</div>';
            console.error(error);
        }
    }
